...
def get_chi(residuetype):
    ...
    if residuetype.upper() not in chi_atoms :
        sys.stderr.write("Warning: Unknown residuetype "+residuetype+" in CalcAngles.get_chis() \n")
        return {}
    #
...
def calc_chi_angles(residue, silent=False):
    ...
    for x in angles:
        ...
        if (xa1 == "@@" or xa2 == "@@" or xa3 == "@@" or xa4 == "@@" ): 
            if not silent :
                sys.stderr.write("Warning: Not enough Atoms found for "+x+" in "+residue.get_resname()+" \n")
            continue
